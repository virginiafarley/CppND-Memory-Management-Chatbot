...
ChatLogic::~ChatLogic() {
  ...
  // TASK 5: ChatLogic non-owner relation to _chatbot
  // delete chatbot instance
  // delete _chatBot;

  // TASK 3: Adapt _nodes so ChatLogic exclusively owns GraphNode instances
  // delete all nodes
  //   for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it) {
  //     delete *it;
  //   }

  // TASK 4: GraphNode instances exclusively own outgoing GraphEdges
  // delete all edges
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
      ...
      if (type != tokens.end()) {
        ...
        if (idToken != tokens.end()) {
          ...
          if (type->second == "NODE") {
            ...
            // TASK 3: ChatLogic exclusively owns GraphNode instances
            // check if node with this ID exists already
            auto newNode =
                std::find_if(_nodes.begin(), _nodes.end(),
                             [&id](std::unique_ptr<GraphNode> &node) {
                               return node->GetID() == id;
                             }); // pass unique pointer by reference; do not
                                 // transfer ownership

            // create new element if ID does not yet exist
            if (newNode == _nodes.end()) {
              _nodes.emplace_back(std::make_unique<GraphNode>(
                  id)); // add unique pointer to _nodes via factory function
          ...
          if (type->second == "EDGE") {
            ...
            if (parentToken != tokens.end() && childToken != tokens.end()) {
              ...
              // TASK 4: Make edge a unique pointers
              // create new edge
              std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);

              // TASK 3: ChatLogic exclusively owns GraphNode instances
              edge->SetChildNode((*childNode).get());   // use raw pointer
              edge->SetParentNode((*parentNode).get()); // use raw pointer
              ...
              (*childNode)
                  ->AddEdgeToParentNode(
                      edge.get()); // TASK 4: GraphNode instances hold
                                   // non-owning reference to incoming
                                   // GraphEdges
              (*parentNode)
                  ->AddEdgeToChildNode(
                      std::move(edge)); // TASK 4: GraphNode instances
                                        // exclusively own outgoing GraphEdges
            }
  ...
  for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it) {
    ...
    if ((*it)->GetNumberOfParents() == 0) {
      ...
      if (rootNode == nullptr) {
        ...
        rootNode = (*it).get(); // TASK 3: assign current node to root
      } else {
  ...
  // set ChatBot data handle _chatBot (no owner relation)
  SetChatbotHandle(&chatBot);

  // move semantics to pass ChatBot instance into root node
