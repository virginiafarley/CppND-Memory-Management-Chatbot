...
// TASK 2: Implement Rule of Five
// Use initializer list (C++ guidelines)
ChatBot::ChatBot(const ChatBot &source)
    : _currentNode(source._currentNode), _chatLogic(source._chatLogic),
      _rootNode(source._rootNode) {
  std::cout << "ChatBot Copy Constructor" << std::endl;

  // data handles (owned): exclusive ownership
  this->_image =
      new wxBitmap(*source._image); // deep copy with wxBitmap copy constructor

  // set non-owned data handles for destination ChatBot instance
...
ChatBot &ChatBot::operator=(ChatBot &&source) {
  ...
  // data handles (owned): transfer ownership
